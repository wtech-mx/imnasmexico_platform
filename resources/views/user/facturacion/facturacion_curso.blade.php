...
<section class="primario bg_overley margin_home_nav" style="background-color:#F5ECE4;" id="cafeteria">
    <div class="row">
        ...
        <div class="col-12 col-md-12  m-auto">
            <h1 class="text-white text-center titulo mt-5 " style="color:#836262!important;">Facturacion Cursos</h1>
...
$(function(){
  ...
  if (!/^\d{10}$/.test(telefono)) {
    ...
    $.ajax({
      url: '{{ route("facturacionCurso.search") }}',
      data: { folio,telefono },
      success(response) {
...
$(document).on('submit', '#facturaForm', function(e){
  ...
  $.ajax({
    ...
    success(response) {
      ...
      // 1) Oculta el formulario si quieres
      $('#FacturaContainer').fadeOut();

      // 2) Actualiza la fila correspondiente:
      const row = $(`#nota-row-${response.order_id}`);
      // Cambia el estatus
      row.find('.status-col').text('Facturado');
      // Inserta (o reemplaza) el botón de descarga
      row.find('.actions-col').html(`
        <a href="${response.pdf_url}"
           download="Factura_${response.order_id}.pdf"
