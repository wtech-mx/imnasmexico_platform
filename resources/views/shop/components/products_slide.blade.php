    ...
    <div class="row">
        ...
        <div id="popularProductsCarousel" class="owl-carousel owl-theme">
            ...
            <div class="container_grid row" id="producto-{{ $producto_popular->id }}">
                ...
                <div class="col-12">
                    <h5 class="text-left m-1 brand_text_grid">{{ $producto_popular->Marca->nombre ?? 'SM' }}</h5>
                    ...
                    @if($producto_popular->ProductoStock->precio_descuento == NULL)
                        @if(date('N') == 1 && $producto_popular->id_categoria == 26)
                            <h6 class="price_text_grid_desc">
                                ...
                                a ${{ number_format($producto_popular->ProductoStock->precio_normal, 2, '.', ',') }}
                            </h6>
                        @else
                            <h6 class="price_text_grid">${{ number_format($producto_popular->ProductoStock->precio_normal, 2) }}</h6>
                        @endif
                    @else
                        @if(strtotime($producto_popular->ProductoStock->fecha_fin_desc) >= strtotime(date('Y-m-d')))
                            <h6 class="price_text_grid_desc">
                                <strong class="precio_reaja">de <s>${{ number_format($producto_popular->ProductoStock->precio_normal, 2, '.', ',') }}</s></strong>
                                a ${{ number_format($producto_popular->ProductoStock->precio_descuento, 2, '.', ',') }}
                            </h6>
                        @else
                            <h6 class="price_text_grid">${{ number_format($producto_popular->ProductoStock->precio_normal, 2) }}</h6>
                        @endif
