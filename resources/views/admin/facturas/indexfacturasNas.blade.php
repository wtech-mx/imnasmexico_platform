            ...
            <div class="table-responsive">
                <table class="table table-flush" id="datatable-search">
                    ...
                    <tr>
                        ...
                        <td>{{ $factura->NotasNas->User->name }}</td>
                        <td>{{ $factura->NotasNas->User->telefono  }}</td>
                        <td>#{{ $factura->NotasNas->id }}</td>

                        <td>
                            {{ $factura->NotasNas->folio }}
                        </td>
                        ...
                        <td>
                            @php
                                 $precio = number_format($factura->NotasNas->total, 2, '.', ',');
                            @endphp
                            $ {{ $precio }}
                        </td>
                        ...
                        <td>
                            ...
                            @can('factura-compra')
                                <a target="_blank" class="btn btn-sm btn-warning" href="{{ route('pagos.edit_pago',$factura->NotasNas->id) }}">
