...
class MultiFacturaController extends Controller
{
    ...
    public function CFDI_facturaDeContado($id_orden)
    {
        ...
        $orden = Orders::with(['User', 'orderTickets.Cursos'])->find($id_orden);
        ...
        foreach ($orden->orderTickets as $op) {
            if($op->cantidad == NULL){
                $canti = 1;
            }else{
               $canti = $op->cantidad;
